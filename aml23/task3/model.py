...
class ViTBackbone(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=1024, depth=24, num_heads=16,
                 decoder_embed_dim=512, mlp_ratio=4., norm_layer=nn.LayerNorm, drop_path=0.1):
        super().__init__()
        self.in_chans = in_chans
...
class SegmentationModel(nn.Module):
    def __init__(self, 
                 ...
                 fpn_inplanes=[512, 512, 512, 512],
                 fpn_dim=256,
        ):
        super().__init__()
        self.vit_backbone = vit_backbone
        self.cls_token = self.vit_backbone.cls_token
        self.patch_embed = self.vit_backbone.patch_embed
        self.pos_embed = self.vit_backbone.pos_embed
        self.blocks = self.vit_backbone.blocks
        self.embed_dim = self.vit_backbone.embed_dim
        self.num_fmap = num_fmap
        self.out_indices = out_indices
        
        # Deconvolutional layers for upscaling
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.out_indices:
                ...
                # append CLS token
                cls_token.append(x[:, 0, :].unsqueeze(1))
        
        cls_token = torch.cat(cls_token, dim=1)
        cls_token = cls_token.sum(dim=1)
        ...
        f = self.fpn_in[-1](features[-1])
        f = self.fpn_out[-1](f)
        fpn_feature_list = [f]
        for i in reversed(range(self.num_fmap - 1)):
            conv_x = features[i]
        ...
        mask = F.interpolate(mask, mask.shape[2] * 4, mode='bilinear', align_corners=False)
        bbox_pos = self.bbox_predictor(cls_token)
        return mask, bbox_pos